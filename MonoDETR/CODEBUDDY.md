...
所有新代码必须用方向前缀标注：`# [P1-SCA-FPN]`、`# [P2-KHYP]`，方便 thesis-final 合并时识别归属。
...
评测方法论：采用论文标准做法，以 checkpoint_best（按 Car Mod AP3D R40 选帧）的结果为最终汇报数据。内部消融判断时注意：Ped/Cyc 在 checkpoint_best 帧受选帧噪声影响（历史实验中单点漂移达 ±2 AP），建议确认主结果经多 seed 验证的稳定性。
